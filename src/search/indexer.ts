...
export function indexAll(db: Database, root: string = DEFAULT_SESSIONS_ROOT): IndexStats {
  const files = walkSessions(root);
  const stats: IndexStats = { filesScanned: files.length, filesChanged: 0, messagesIndexed: 0, bytesIngested: 0 };
  for (const f of files) {
    const r = indexFile(db, f);
...
const upsertSession = (db: Database) => db.prepare(
  `INSERT INTO sessions (session_id, file_path, project, cwd, started_at, last_msg_at, msg_count, first_user_prompt)
   VALUES (?, ?, ?, ?, ?, ?, ?, ?)
...
const insertFts = (db: Database) => db.prepare(
  `INSERT INTO messages_fts (session_id, msg_index, ts, user_text, assistant_text, tool_calls, file_paths)
   VALUES (?, ?, ?, ?, ?, ?, ?)`
);
...
export function indexFile(db: Database, filePath: string): { messagesIndexed: number; bytesIngested: number } {
  const st = statSync(filePath);
  ...
  if (st.size < startOffset) {
    ...
    return reindexFromScratch(db, filePath);
  }
  if (st.size === startOffset) {
  ...
  const txn = db.transaction((lines: string[]) => {
    ...
    upsertSession(db).run(
      ...
      startedAt, lastMsgAt, indexed, firstPrompt,
    );
...
function reindexFromScratch(db: Database, filePath: string): { messagesIndexed: number; bytesIngested: number } {
  const sessionId = sessionIdFromPath(filePath);
